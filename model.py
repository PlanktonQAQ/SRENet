...
class enc_net(nn.Module):
    ...
    def forward(self, pre, img_input):
        res_sal = self.r_net(img_input*pre)
        res_bk = self.r_net(img_input*(~pre))
        # res_sal = self.gelu(res_sal)*pre
        # res_bk = self.gelu(res_bk)*(~pre)
        res_sal = self.gelu(res_sal)
        res_bk = self.gelu(res_bk)
        res_f = torch.clamp(res_sal+res_bk, 1e-9, 1.0)
        res = self.d_net(res_f)
        return res_sal, res_bk, res_f, res
...
class cma(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels, out_channels, 3, 1, 1), nn.InstanceNorm2d(out_channels), nn.LeakyReLU(0.2, inplace=True))
